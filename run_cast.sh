...
out_caster_A="-msg ${rtcm_msg_a} ntrips://:${svr_pwd_a}@${svr_addr_a}:${svr_port_a}/${mnt_name_a}#rtcm3 -p ${position}"
#add receiver options if it exists
[[ ! -z "${ntrip_a_receiver_options}" ]] && out_caster_A=""${out_caster_A}" -opt "${ntrip_a_receiver_options}""

out_caster_B="-msg ${rtcm_msg_b} ntrips://:${svr_pwd_b}@${svr_addr_b}:${svr_port_b}/${mnt_name_b}#rtcm3 -p ${position}"
#add receiver options if it exists
...
mkdir -p ${logdir}
  ...
  case "$2" in
    ...
    ${cast} -in ${!1} ${out_tcp} -b 1 -t ${level} -fl ${logdir}/str2str_tcp.log &
    ;;
